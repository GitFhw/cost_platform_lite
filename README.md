...
数据库有两种落位方式，二选一即可；集成时必须先声明是“分离版本”还是“同库版本”：

1. 分离版本：宿主业务库不初始化、不改动任何 `cost_*` 核算表；计费服务连接独立核算库。若独立核算库是空库，按计费服务自身的部署流程初始化，不能把这一步带入业务项目迁移或业务库脚本。
2. 同库版本：计费 Starter 复用宿主数据源，必须在客户业务数据库中执行对应 SQL 初始化 `cost_*` 表和必要字典；业务项目原有业务表无需迁移。
...
轻量包不把“动态列”当成一张可以任意手填的宽表，而是由费目要素元数据驱动编辑器和执行链。宿主只需要提供变量编码、数据类型、来源类型和选项来源，工作台会按以下契约渲染并在服务端复核：

- 平台字典、业务字典和业务主数据使用稳定编码；低基数字典可在矩阵中用下拉选择，`IN/属于任一` 支持多选，保存仍是可审计的编码集合。高基数、分页不完整或选项快照不可信时自动留在高级规则，不允许客户靠手工输入绕过字典。
- 数值要素使用数值输入和行级比较操作符；普通文本使用文本输入；布尔使用是/否；类型不明确时前后端均收紧可用操作符，避免“页面能配、运行不命中”。矩阵当前只承载低基数、易审计的 `EQ/IN` 字典、布尔 `EQ`、数值比较和文本 `EQ`，复杂语义进入高级规则。
- `DATE` 表示自然日，`DATETIME/TIMESTAMP` 保留到秒，`TIME_OF_DAY` 表示每日时刻；日期/时间支持 `EQ/GT/GE/LT/LE/BETWEEN`，每日时段允许跨午夜且起止相等表示单点时刻。预演、发布校验和正式运行共用类型化比较器。
- `TIER_RATE` 采用当前轻量包的“计量值命中一档区间，再按该档单价乘数量”模型；区间连续性、边界、空档和重叠由前后端校验。母体旧页面中的 `eachDay/addRate` 等渐进累计字段不属于轻量包契约，不能混入同一规则类型。

因此迁移到任意宿主时，接入方只需实现 `CostLiteApi` 和业务字典选项提供器，并声明同库或分离库模式；计费核心不要求复制母体工程，也不会把客户业务表或数据库连接写进 Jar。MySQL、Oracle 共用这套前端和规则协议，数据库差异由各自 Starter/核心版本和方言兼容层收口。
