...
/**
 * 根据显式模式选择计费数据源；未配置模式时安全默认使用独立核算库。
 *
...
public final class CostLiteDataSourceModeCondition {
    ...
    private static String mode(ConditionContext context) {
        ...
        if (configured == null || configured.trim().isEmpty()) {
            configured = context.getEnvironment().getProperty("COST_LITE_DATASOURCE_MODE", "");
        }
        ...
        return normalized.isEmpty() ? MODE_DEDICATED : normalized;
    }
}
